...
- Speaker identification labels only the matching cluster as `BP`; non-matching voices remain generic as `Other speaker` or `Unknown speaker`.
...
Runtime diarization in this smallest complete version is embedding-based per-ASR-segment verification. Anonymous clusters may still be used internally to reduce false BP matches, but the product only exposes BP vs not-BP because the goal is recognizing BP, not identifying everyone else. It does not yet run a full diarization model, so overlapping speakers and sub-segment speaker turns remain a known limitation. The design keeps the storage/API boundary ready for a full diarization backend once dependencies are installed and model access is confirmed.
